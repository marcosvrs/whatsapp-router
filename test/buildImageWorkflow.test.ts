...
describe("image publishing workflow", () => {
  it("only publishes workflow-run images from successful main workflow completions", () => {
    expect(workflow).toMatch(
      /github\.event_name == 'workflow_run'[\s\S]*github\.event\.workflow_run\.head_branch == 'main'[\s\S]*github\.event\.workflow_run\.conclusion == 'success'/,
    );
  });
